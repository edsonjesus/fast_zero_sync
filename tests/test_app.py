...
def test_read_user_deve_retornar_usuario(client):
...
def test_read_user_deve_retornar_erro_404_quando_usuario_nao_existe(client):
...
def test_update_user_deve_atualizar_nome_e_email_do_usuario(client):
    response = client.put(
        '/users/1',
        json={
...
def test_delete_user_deve_excluir_usuario(client):
